...
- **不要：** 复活导入 delayCall；跳过 [1]；给管线加人工平铺入口；合并 Plan 进 Orchestration；按行数再拆 3000 行内核；插件 1 Load 管线步骤 SO。
- 质量闸真样（leftover 41 / 内嵌 Extract / 短名 42）**调试暂放**，见 [backlog](../03_open-items/backlog.md)。
