...
2026-09-14本地核对：`main`代码基线为 **e51469a**（后处理材质透明问题修复），上一提交 **8eb6612**（插件1拆分初步）；本轮开始时工作区干净。此前“151条未提交”的状态已过时；本轮仅文档修订，尚未提交。**v1.5.3是历史发布标签**，对应 `8131b31`，不等于当前main。

功能分支 `feature/cli-pipeline-2022` 停在 `2b13690`（已合入 main 的 #9）。之后工作直接在 **main**。

**D5** 无头已于 2026-09-02 验收（`直18.gltf` → `exit=0`）；契约见 [cli-getting-started](../04_implementation/cli-getting-started.md)。下一步产品向：AB 文件名见 [§2](#2-issue-274)（未改代码）。
...
**截至2026-09-02记录，之后工单上只有一条（本轮未重查远程）：**
...
截至该次历史核对，没有新的 APP 书面确认「必须改文件名」。因此 D1 契约 1 **仍退化**；插件侧 2026-09-02 只做了**评估**（未开发）：
...
| 日期 | 哈希 | 说明 |
|---|---|---|
| 2026-09-14 | （文档，未提交） | 需求整理：D13-R1 移动端关闭；缺伴生整趟停；④目录暂留；OBJ 不进 MissingUris；B 质量闸待 R3；历史规则按文件收缩 |
