...
**怎么跑：** 插件根 [README · 操作者入口](../../../README.md)（总面板格号、CLI、退出码）。本文讲三条「自动」和实现要点，不是操作手册。
...
`Assets/Art/**` = ④平铺写出的**交付单元**。实现物理位于插件 2，ctx 驱动的相位职责暂归中间层；资源处理另有三条通道，都叫「自动」时最容易混：
...
| ④ 平铺 | 管线默认开；人工走 `Tools > Retinar > 批量汇总` 下“普通平铺”或“原子迁移”，两者均执行完整④。配置来自各自作用域的平铺 SO |
| ⑤ 总批量 | `Tools > 资源处理总面板`：路径直指 Art；顺序 **贴图 → 材质(Shader) → 模型**（Pipeline **默认开**） |
| ⑥ / 日常出包 | 管线⑥（读 `RetinarExportSettings`） |
| 已删除入口 | “成品直达”与“从 Art 规范化导出”均已删除（D24-7），不得作为现行操作说明 |
