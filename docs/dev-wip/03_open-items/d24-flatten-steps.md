...
按钮 → `plan.Branch`。B′ sidecar 用 `GltfPackageFiles.Scan`，不是 `JobContext.Build`。选中 `.gltf` 直接 Scan；Prefab 用 `GetDependencies` 找 `.gltf` 再 Scan。FBX/OBJ 不 Scan、无提示。点「普通平铺」且有相对 URI：弹窗确认；**仍要平铺**才继续，**取消/叉号中止**。

调度/操作分目录（中间层 `Pipeline/Editor/Flatten/`），未搬 3518 行内核。
...
第 10 步 CLI 已核；第 11 步用户常规测试暂无问题，真实重名专项仍未确认。第 12 步仅清理停用代码；第 13 步剩余接口需另核对，第 14 步 `.fbm` 质量闸仍未拍。
