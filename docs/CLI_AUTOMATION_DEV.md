# 命令行自动化一体流程 — 开发说明（入口）

> **分册已迁至：** [`docs/dev-wip/README.md`](./dev-wip/README.md)  
> 请到该目录查阅：需求/战略、结构、待办、实现流程。  
> 本文保留为兼容旧链接的短入口；细节以 `dev-wip` 为准。

| 分册 | 链接 |
|---|---|
| 目录 | [dev-wip/README.md](./dev-wip/README.md) |
| 已确认战略 | [dev-wip/01_requirements/strategy.md](./dev-wip/01_requirements/strategy.md) |
| 技术与操作 | [dev-wip/01_requirements/tech-and-ops.md](./dev-wip/01_requirements/tech-and-ops.md) |
| 代码结构 | [dev-wip/02_structure/overview.md](./dev-wip/02_structure/overview.md) |
| 待处理/模糊 | [dev-wip/03_open-items/backlog.md](./dev-wip/03_open-items/backlog.md) |
| 实现流程 | [dev-wip/04_implementation/pipeline-flow.md](./dev-wip/04_implementation/pipeline-flow.md) |

分支：`feature/cli-pipeline-2022` · 工单：http://swm-server.local:3000/Admin/prd-docs/issues/274
