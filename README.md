...
| [`TOol/`](./TOol/) | 批量 FBX 入库；导入期设置 + 源文件/模型后处理；物理承载③ Prefab 与④平铺内核 | `Tools > 批量FBX导入`；`Tools > 资源处理总面板` |
...
3. **人工执行完整④**：选中 Prefab/模型后，按数据形态选择“普通平铺（B）”或“原子迁移（B′）”。两个入口都会继续完成 E?/D/C/Finish。  
   ...
   - **直接选 FBX 的人工普通入口**：仍走 SafeZone；管线③产出的 Prefab 不因此进入这条兼容分支。\
   - **相对 URI glTF**：必须走原子迁移；缺伴生直接失败。  
