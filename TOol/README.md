...
- **与打包插件关系：** **导入期自动**默认不碰 `Assets/Art/`（`excludedPathPrefixes`）；**⑤ 总批量 / L1** 默认路径正是 Art，平铺后对交付区压图/刷白。交付打包见 `../RetinarBatchBuilder_Share/`；批量入库不自动建 Prefab，交付名仍以人工 Prefab 名为准。细节见 [ARCHITECTURE.md](./ARCHITECTURE.md)「配置归属」与「介入目录」。
