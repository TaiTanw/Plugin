...
public class ShrinkTextureSourceOperation : ITextureAssetOperation
{
    ...
    public TextureOperationResult Execute(TextureOperationContext context)
    {
        ...
        if (AssetPathUtility.IsInsideEmbeddedMediaFolder(context.AssetPath))
        {
        ...
        string fullPath)
    {
        ...
        if (best.Bytes.LongLength >= originalBytes.LongLength)
        {
            ...
            return TextureOperationResult.Skipped(
                "重新编码后体积没有变小（" + AssetPathUtility.FormatBytes(originalBytes.LongLength) +
                " -> " + AssetPathUtility.FormatBytes(best.Bytes.LongLength) + "），保留原文件。");
        }
        ...
        return TextureOperationResult.Changed(
            AssetPathUtility.FormatBytes(originalBytes.LongLength) + " -> " +
            AssetPathUtility.FormatBytes(best.Bytes.LongLength) + "，尺寸 " +
        ...
        int encodeCount)
    {
        ...
        return EncodeAttempt.Failure(
            ...
            AssetPathUtility.FormatBytes(fallback.Bytes.LongLength) + "，超过阈值 " +
            AssetPathUtility.FormatBytes(context.Settings.MaxSourceBytes) + "。" +
