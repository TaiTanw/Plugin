...
public class ConvertTgaToPngOperation : ITextureAssetOperation
{
    ...
    public bool CanProcess(string assetPath, TextureProcessSettings settings)
    {
        ...
        return TextureAssetPathUtility.GetFileLength(assetPath) >= 0;
    }
    ...
    {
        string tgaFullPath = TextureAssetPathUtility.ToFullPath(context.AssetPath);
        ...
        byte[] pngBytes)
    {
        ...
        string message = TextureAssetPathUtility.FormatBytes(tgaLength) + " (TGA) -> " +
            TextureAssetPathUtility.FormatBytes(pngBytes.LongLength) + " (PNG) " + pngAssetPath;
