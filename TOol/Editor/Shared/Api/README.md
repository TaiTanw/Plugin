...
| `ToolFlattenApi` | ④ | `FromContext(ctx, request, prefab)` → `Run(plan)`；人工自己组 plan，不建 ctx。兼容分步口仍保留，旧 `FlattenPaths` 已在步骤 12 删除 |
...
④虽物理在插件2，消费ctx的API/服务/编排暂按职责归中间层。自动每模型②.5一次；人工仅轻扫描伴生事实并组 plan，禁止另建 ctx。内核七步传 Work，步骤 12 未迁移目录或移除现用七步。详见[整体结构](../../../../docs/dev-wip/02_structure/overview.md)。
