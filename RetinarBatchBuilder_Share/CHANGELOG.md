...
## 2026-09-23 — v1.7.0：⑥ 可选模型文件与资源信息表

- 原因：手动导出 `Assets/Art/测试0002/Prefab/测试0002.prefab` 后，模型文件和顶点/三角面是对的，但表上仍印着模板样例：Renderer 144、材质 14、12 张 2048 贴图，以及「FBX 已交付」。
- 改动：这些格子改为从该预设体统计，或改成待补充。DCC 与版权段里点名 FBX、预览图、演示视频的样例句子改为待补充。
- 影响：当前推荐线 **v1.7.0**。已经打出的 `Deliverables/测试0002/06_docs/asset_info.xlsx` 仍是收尾前的内容，需要再导出一次才会换成统计值。仓库里的导出 SO 保持「拷 AB 到交付夹」开、模型文件与资源表关。
- 验证：该次手动导出本身成功。双端 AB 约 6.3MB / 6.2MB，`02_unity/测试0002.unitypackage` 约 2.1MB。`01_source/Model` 共 10 个文件：`直18.gltf`、`直18.bin`，以及 glTF 相对 URI 点名的 8 张 jpg，没有缺伴生日志。表上 A1/B5 为测试0002，B17 为 2022.3.54f1c1，B25 为 59147，B26 为 66147，B34 为 `Assets/Art/测试0002/Prefab/测试0002.prefab`。同一份表在收尾前仍印着模板的 Renderer 144、材质 14、12 张贴图和「FBX 已交付」。本环境未再打一包。
- 回退：恢复只填 B17/B25/B26/B34。
- 关联问题：无。
- 文档：操作者须知、D1、相位入参写明导出设置「拷贝 AB」不跳过构建。取消后双端仍打到 AB 根；勾选的 UP / 模型 / 资源表在构建成功后照写，交付夹可以没有 `03_assetbundles`。管线关掉步骤⑥则 `Build` 不调用。

## 2026-09-23 — v1.7.0 窄口：模型文件与资源信息表

- 原因：交付需要在双端 AB 之外逐步加回模型文件和顶点统计表。⑥ 只收预设体，不能按 Art 夹名找，也不能把已删的全套导出做成开关。
- 改动：
  1. 导出 SO / `RetinarAbBuildOptions` 增加 `exportSourceModels`、`exportAssetInfo`，默认关。`FromExportSettings` 带上。管线不另做面板闸。
  2. `Build` 在该预设体双端 AB 成功后调用 `RetinarOptionalDeliverables`。模型从预设体依赖按后缀拷到 `01_source/Model`；`.gltf` 再跟相对 URI。`.obj` 只拷容器。
  3. 资源表拷 `asset_info_template.xlsx`，只改资源信息页的值格，清掉空警-2000 样例身份。
  4. 这两项失败只打日志，不进 `FailLines`。
- 影响：默认关时出包与 v1.6.8 相同。随 **v1.7.0** 发布。
- 验证：文档与窄口对齐。手动导出核对见上一条。
- 回退：两开关保持关；或去掉 `WriteAfterAb` 调用。
- 关联问题：无。

## 2026-09-22 — v1.6.8：④ 同名图不互盖，身份闸本样已关

- 原因：pack 丢掉脚本后④写不出 Art；同名两张图会被当成一张互盖或报 42。测试1112 清空重导要能 `exit=0`。
- 改动：
  1. **管线④** 写入 Art 副本前剥 Missing Script（Incoming 不改）。人工④默认不剥，缺脚本则失败并列出。
  2. **落点：** `.fbm` 父夹固定套到 `image/Texture/<名>.fbm/`。分类根已被占用且来源父夹不是分类叶名时才再套一层。同名已占坑不覆盖。
  3. **认账：** 字节相同才入账。模型重绑只在本单元已有依赖路径里定一张；同名两条以上都在 `.fbm` 则警告，不猜。材质上已指向的平铺图不改。
  4. **文档：** 操作者须知写明两张同名图、Unknown、空槽、⑥ 后顶点色日志。主页写明手机端对 2022 所导出、带 unitypackage 动画的 AB 会闪退。
- 影响：曾为推荐线；**现由 v1.7.0 接替**。纯模型 2022 / 2020 都可导出。带 unitypackage 动画等资源时，手机端加载 2022 导出的 AB 会闪退；同一套代码迁到 2020 导出已确认可加载。
- 验证：测试1112 清空旧 Art / Incoming 再跑，`exit=0`；Character01 与 Eye_Eyeball 的 `textureIdentityWarnings=0`、`leftoverFbm=0`。`Child.fbx` 的 Face/Body 绑在套层图，平铺材质仍指平铺文件。本环境未跑 Unity EditMode。
- 回退：回退至标签 `v1.6.5`。
- 关联问题：prd-docs#274。

## 2026-09-21 — v1.6.5：unitypackage 第一刀

- 原因：要把别人打好的 `.unitypackage` 接进管线，但不能走 Unity Import Package（原址、脚本、GUID）；操作者入口也容易和选择器/多选拖入混在一起。
- 改动：
  1. **预处理：** 工程外解 tar，丢掉脚本/dll/场景，带其余 meta 写到调用方导入根 `<ID2>/` 信封。编排只调 `ToolImportApi`，pack 不进模型扩展名表。
  2. **展开：** 信封内只按根 Prefab 拆行；无根 → 入库 20。③ 已是 Prefab 则原样交。④⑥ 仍按 Prefab 文件名。
  3. **入口：** 编排「浏览…」单选是面板上 **唯一** up 入口（表上只一条 pack）。灰框多选、选择器文件夹批量只认模型。CLI `-source` 可给一个 pack，同一套 Runner。
  4. **⑤：** 交付材质若已是 Unity 内置 Shader，或 Shader 落在同一 Art 单元，则不烤成 Standard。
  5. **文档：** 操作者须知写清三种导入，并说明不要把 up 拖进 Project。
- 影响：曾为推荐线；**现由 v1.6.8 接替**。选择器仍不收 pack。误把 up 拖进 Project 不是管线 [1]。
- 验证：EditMode 含解包 / 展开 / CLI pack；本机 CLI 跑过带嵌套空 Prefab 的样包。本机未重跑无头全流程。
- 回退：回退至标签 `v1.6.2`（会一并丢掉 `613cb97` 亮度转透明修复）。
- 关联问题：prd-docs#274。

## 2026-09-20 — v1.6.2：逻辑优化

- 原因：人工与管线配置仍会串；无条件全池的贴图 Op 不能进总批量；跑完工作根需要可勾选清空。
- 改动：
  1. **路径：** 人工三根根进选择器 SO；管线仍读步骤 SO。互不跟随。
  2. **⑤：** `AllowMasterBatch=false` 不得进 L1 / 管线⑤ / 导入自动（现网亮度→Alpha）；扫描跟勾选 Op 的 Evaluate。
  3. **清空：** 步骤 SO 可清 Incoming+IncomingPrefab / Art；先清 Art，一次 Refresh。CLI 跟 SO，Quiet 仍强制。
  4. **OBJ：** 选择器轴向勾选可带到编排表；④ Finish 叠 −90°X。
  5. **文档：** 操作者须知草稿；开发者须知按现约压缩。
- 影响：当前推荐线 **v1.6.2**。亮度转透明只能精准面板勾。
- 验证：EditMode 含清空 / OBJ 轴向保留；本机未重跑无头全流程。
- 回退：回退至标签 `v1.6.0`。
- 关联问题：prd-docs#274。
...
## 2026-09-02 — v1.5.3：gltf 整包平铺 + 管线重名夹级覆盖

- 原因：v1.5.0 之后要把 `.gltf` 多文件包和「同槽换内容」收成可发布基线；AB 工单文件名仍待 APP，本版不改。
- 改动：
  1. **②③④ glTF（D23）：** 工程外 `.gltf` 整包入库（Scan URI 伴生）；`HasExternalUris` 时④ 走 B′，落到 `Art/<名>/<名>/`，不拆相对路径。
  2. **D18：** 管线重名只清本趟 `Incoming/<三层>/`、`Art/<名>/` 再写；菜单平铺 / 批量导入仍 Skip。不扫整棵 Incoming/Art。
  3. **文档：** `docs/dev-wip/05_dev-log/timeline.md`（提交次第 + #274 + Plugin 仓对话）；D1 文件名评估（夹不变、夹内两工单名，未改代码）。
  4. 流程/文档小步（`71fb839`、`ca15617`）。
- 影响：当前推荐线 **v1.5.3**。AB `{id}_android/_ios` 文件名、契约 2 `main` **未进本版**。无头 D5 代码已在管线内，验收见同日文档收口。
- 验证：编辑器 `直18.gltf` 全流程 `exit=0`；D18 同槽再跑会换新内容。
- 回退：回退至标签 `v1.5.0`（无 B′ / 无管线夹级清空）。
- 关联问题：prd-docs#274；相对 v1.5.0：`71fb839` … `c9320ec`。
...
- 影响：曾为推荐线 **v1.5.0**；现由 **v1.5.3** 接替。无头 / Docker / UnityGLTF 宿主锁文件等仍按 backlog 推进，不阻断本版编辑器流程。
