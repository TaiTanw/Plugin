...
| 管线 ⑥ `RetinarAbApi` | 出包 | **不改** Art | `03_assetbundles`；导出 SO 可另开 `02_unity`、`01_source/Model`、`06_docs/asset_info.xlsx`（后两项默认关） |

2026-09-03（backlog D24-7）已删：「【遗产】从 Art 规范化导出」「成品直达」、`RetinarPackageScheduler`、`RetinarDirectPackage`、出包前三道校验、`30_Business` 整层。不再写 `00_` / 贴图体积报告。`01_source/Model` 与 `06_docs/asset_info.xlsx` 只在导出 SO 打开时，由 `Build` 在 AB 成功后另写。
...
Assets/Retinar/Editor/
  ...
  40_Api/
    RetinarAbApi.cs            ⑥ 窄口
    RetinarOptionalDeliverables.cs  默认关：模型文件与资源信息表
...
出包：管线步骤⑥开着才进 RetinarAbApi.Build
  → 先双端 AB 到 AB 根；「拷到交付夹」开着才写入 03
  → 构建成功且导出 SO 打开时再写 02_unity / 01_source/Model / 06_docs
  → 步骤⑥关掉则以上都不跑
```
